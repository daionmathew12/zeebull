...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/tmp/attendance_debug.log'),
...
def clock_in(clock_in_data: ClockInCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    try:
        logger.info(f"Clock-in request received for employee_id={clock_in_data.employee_id}, location={clock_in_data.location}")
        ...
        logger.info(f"Checking for existing open clock-in for employee {clock_in_data.employee_id} on {now.date()}")
        ...
        if open_log:
            logger.warning(f"Employee {clock_in_data.employee_id} already has an open clock-in")
            raise HTTPException(status_code=400, detail="Employee is already clocked in. Please clock out first.")

        logger.info(f"Creating new WorkingLog entry")
        new_log = WorkingLog(
        ...
        logger.info(f"WorkingLog object created: {new_log}")
        
        db.add(new_log)
        logger.info("Added to session, committing...")
        db.commit()
        logger.info("Committed successfully, refreshing...")
        db.refresh(new_log)
        logger.info(f"Clock-in successful for employee {clock_in_data.employee_id}, log_id={new_log.id}")
        return new_log
    except HTTPException:
        logger.warning("HTTPException raised, re-raising")
        raise
    except Exception as e:
        logger.error(f"Error in clock_in: {type(e).__name__}: {str(e)}")
