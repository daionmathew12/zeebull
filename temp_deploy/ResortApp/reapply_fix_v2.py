...
# New process_batch definition (Safe Access)
new_process_batch = [
...
# New calling logic (With Definitions)
new_call_logic = [
    '                # 2. Process Rented and Standard Good batches\n',
    '                # Determine if Asset or Consumable\n',
    '                is_asset_type = False\n',
    '                try:\n',
    '                    if item.category and item.category.type == "asset":\n',
    '                        is_asset_type = True\n',
    '                except:\n',
    '                    pass\n',
    '                \n',
...
for i, l in enumerate(lines):
    if 'def process_batch(' in l:
        b1_start = i
    # Identify end of process_batch block. 
    # It ends before 'if permanently_mapped_qty > 0:'
...
b2_end = -1
for i, l in enumerate(lines):
    # Depending on previous runs, we look for '# 2. Process Rented' or 'if is_asset_type:'
    if i > 500:
        if '# 2. Process Rented' in l or 'if is_asset_type:' in l:
            if b2_start == -1: b2_start = i
...
if b1_start != -1 and b1_end != -1 and b2_start != -1 and b2_end != -1:
    ...
    with open('/tmp/checkout_fixed_final_v2.py', 'w') as out:
