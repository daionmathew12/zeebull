...
def get_categories(
    ...
    categories = inventory_crud.get_all_categories(db, branch_id=branch_id, skip=skip, limit=limit, active_only=active_only)
    
    # Results are already global from CRUD
    result = []
...
async def create_item(
    ...
    try:
        ...
        category = inventory_crud.get_category_by_id(db, category_id, branch_id=branch_id)
        if not category:
            raise HTTPException(status_code=404, detail=f"Category with id {category_id} not found in this branch")
        ...
        try:
            ...
            db.rollback()
            err_msg = str(e).lower()
            if "item_code" in err_msg:
                raise HTTPException(status_code=400, detail="Item Code already exists in this branch. Please use a unique code.")
            elif "barcode" in err_msg:
                raise HTTPException(status_code=400, detail="Barcode already exists in this branch.")
            elif "uix_inventory_item_name_branch" in err_msg or "inventory_items_name_key" in err_msg:
                 raise HTTPException(status_code=400, detail="Item Name already exists in this branch.")
            else:
...
def get_item_comprehensive_details(
    ...
    category = inventory_crud.get_category_by_id(db, item.category_id, branch_id=branch_id)
...
async def update_item(
    ...
    try:
        ...
        if check_cat_id is None:
             current_item = inventory_crud.get_item_by_id(db, item_id, branch_id=branch_id)
             if current_item:
                 check_cat_id = current_item.category_id

        if check_cat_id:
            cat_obj = inventory_crud.get_category_by_id(db, check_cat_id, branch_id=branch_id)
        ...
        category = inventory_crud.get_category_by_id(db, updated.category_id, branch_id=branch_id)
...
def get_purchase(
    ...
    purchase = db.query(PurchaseMaster).filter(PurchaseMaster.id == purchase_id, PurchaseMaster.branch_id == branch_id).first()
...
def update_purchase_status(
    ...
    purchase_check = db.query(PurchaseMaster).filter(PurchaseMaster.id == purchase_id, PurchaseMaster.branch_id == branch_id).first()
...
def create_requisition(
    ...
    try:
        ...
        details = []
        for detail in created.details:
            item = inventory_crud.get_item_by_id(db, detail.item_id, branch_id=branch_id)
            details.append({
                **detail.__dict__,
                "item_name": item.name if item else None
            })
...
def get_location_items(
    ...
    for mapping in asset_mappings:
        item = inventory_crud.get_item_by_id(db, mapping.item_id, branch_id=branch_id)
        if item:
...
def create_asset_mapping(
    ...
    try:
        ...
        item = inventory_crud.get_item_by_id(db, created.item_id, branch_id=branch_id)
...
def get_asset_mappings(
    ...
    for m in real_mappings:
        ...
        item = inventory_crud.get_item_by_id(db, m.item_id, branch_id=branch_id)
...
def update_asset_mapping(
    ...
    item = inventory_crud.get_item_by_id(db, updated.item_id, branch_id=branch_id)
...
def unassign_asset(
    ...
    # Verify mapping belongs to branch
    mapping_check = db.query(AssetMapping).filter(AssetMapping.id == mapping_id, AssetMapping.branch_id == branch_id).first()
...
def get_asset_registry(
    ...
    result = []
    for asset in assets:
        item = inventory_crud.get_item_by_id(db, asset.item_id, branch_id=branch_id)
...
def get_laundry_items(
    ...
    ).filter(LaundryLog.status != "Returned", LaundryLog.branch_id == branch_id).order_by(LaundryLog.sent_at.desc()).all()
