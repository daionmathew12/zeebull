...
class ActivityLoggingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if request.url.path.startswith("/api"):
            ...
            # Default branch fallback to 2 since 1 doesn't exist in your DB
            branch_id = getattr(request.state, "branch_id", 2)
            
            # If branch_id is None (e.g. superadmin Enterprise View), use 2 as a physical anchor
            if branch_id == "all" or branch_id is None:
                branch_id = 2
