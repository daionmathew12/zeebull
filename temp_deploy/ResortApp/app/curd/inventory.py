...
def get_all_categories(db: Session, branch_id: int = None, skip: int = 0, limit: int = 100, active_only: bool = True):
    from sqlalchemy.orm import joinedload
    query = db.query(InventoryCategory)
    # query = db.query(InventoryCategory).options(joinedload(InventoryCategory.branch))
    # if branch_id:
...
def get_category_by_id(db: Session, category_id: int, branch_id: int = None):
...
def get_all_items(db: Session, branch_id: int = None, skip: int = 0, limit: int = 100, category_id: Optional[int] = None, active_only: bool = True, is_fixed_asset: Optional[bool] = None):
    """Optimized with eager loading to prevent N+1 queries"""
    query = db.query(InventoryItem).options(
        joinedload(InventoryItem.category),
        joinedload(InventoryItem.preferred_vendor)
        # joinedload(InventoryItem.branch)
    )
...
def get_item_by_id(db: Session, item_id: int, branch_id: int = None):
...
def update_item_cost_wac(db: Session, item_id: int, new_quantity: float, new_unit_price: float, branch_id: int = None, is_cancellation: bool = False, commit: bool = True):
    """
    ...
    """
    item = get_item_by_id(db, item_id, branch_id=branch_id)
    if not item:
        return None
...
def update_item_stock(db: Session, item_id: int, quantity_change: float, transaction_type: str, branch_id: int = None, unit_price: Optional[float] = None):
    """Update item stock and optionally update price using WAC if type is 'in'"""
    if transaction_type == "in" and unit_price is not None:
        return update_item_cost_wac(db, item_id, quantity_change, unit_price, branch_id=branch_id)
...
def get_all_vendors(db: Session, branch_id: int = None, skip: int = 0, limit: int = 100, active_only: bool = False):
    from sqlalchemy.orm import joinedload
...
def get_vendor_by_id(db: Session, vendor_id: int, branch_id: int = None):
...
def generate_purchase_number(db: Session, branch_id: int):
    ...
    last_entry = db.query(PurchaseMaster).filter(
        PurchaseMaster.branch_id == branch_id,
...
def get_purchase_by_id(db: Session, purchase_id: int, branch_id: int = None):
...
def get_item_stocks(db: Session, item_id: int, branch_id: int = None):
...
def generate_requisition_number(db: Session, branch_id: int):
    ...
    count = db.query(StockRequisition).filter(
        StockRequisition.branch_id == branch_id,
...
def generate_issue_number(db: Session, branch_id: int):
    ...
    while True:
        ...
        exists = db.query(StockIssue).filter(StockIssue.item_id == StockIssue.item_id, StockIssue.branch_id == branch_id, StockIssue.issue_number == candidate).count()
        if exists == 0:
            return candidate
...
def get_all_issues(db: Session, branch_id: int = None, skip: int = 0, limit: int = 100):
    """Optimized with eager loading"""
    from app.models.inventory import StockIssue
...
def create_waste_log(db: Session, data: dict, reported_by: int, branch_id: int = 1):
...
def get_all_waste_logs(db: Session, skip: int = 0, limit: int = 100, branch_id: int = None):
    from app.models.inventory import WasteLog
...
def get_waste_log_by_id(db: Session, waste_log_id: int, branch_id: int = None):
    from app.models.inventory import WasteLog
...
def create_location(db: Session, data: dict, branch_id: int = 1):
    from app.models.inventory import Location
...
def get_all_locations(db: Session, skip: int = 0, limit: int = 10000, branch_id: int = None):  # Increased limit to show all rooms
    from app.models.inventory import Location
    from sqlalchemy.orm import joinedload
...
def get_all_asset_mappings(db: Session, skip: int = 0, limit: int = 100, location_id: Optional[int] = None, branch_id: int = None):
...
def get_all_asset_registry(db: Session, branch_id: int = None, skip: int = 0, limit: int = 100, location_id: Optional[int] = None, status: Optional[str] = None):
...
def get_asset_registry_by_id(db: Session, asset_id: int, branch_id: int = None):
    from app.models.inventory import AssetRegistry
...
def get_all_stock_issues(db: Session, skip: int = 0, limit: int = 100, branch_id: int = None):
    query = db.query(StockIssue).options(
...
def get_all_inter_branch_transfers(db: Session, branch_id: int = None, skip: int = 0, limit: int = 100):
    from app.models.inventory import InterBranchTransfer
