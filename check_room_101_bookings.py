...
with engine.connect() as conn:
    print(f"--- Active Bookings for Room 101 (ID: {room_id}) ---")
    # Check regular bookings
    res = conn.execute(text("""
        SELECT b.id, b.guest_name, b.status, b.check_in, b.check_out
        FROM bookings b
        JOIN booking_rooms br ON b.id = br.booking_id
        WHERE br.room_id = :room_id AND b.status IN ('booked', 'checked-in', 'checked_in')
    """), {"room_id": room_id})
    for row in res:
    ...
    res = conn.execute(text("""
        ...
        WHERE pbr.room_id = :room_id AND pb.status IN ('booked', 'checked-in', 'checked_in')
    """), {"room_id": room_id})
    for row in res:
