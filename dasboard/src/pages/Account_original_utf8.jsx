            ...
            <div className="space-y-4">
              ...
              <div className="p-2.5 rounded-lg bg-white/50 border border-current/10">
                <p className="text-xs font-medium italic">
                  <strong>≡ƒÆí Pro Tip:</strong> {info.tip}
                </p>
              </div>
            </div>
          </div>
    ...
    try {
      ...
      if (dataType === 'inventory') {
        newData = newData.map(item => ({
          ...
          unit_price: item.unit_price != null ? `Γé╣${item.unit_price.toFixed(2)}` : '-',
          stock_value: item.current_stock != null && item.unit_price != null ? `Γé╣${(item.current_stock * item.unit_price).toFixed(2)}` : '-',
  ...
  const handleCreateJournalEntry = async (e) => {
    ...
    try {
      ...
      if (Math.abs(totalDebits - totalCredits) > 0.01) {
        alert(`Journal entry must balance. Debits: Γé╣${totalDebits.toFixed(2)}, Credits: Γé╣${totalCredits.toFixed(2)}`);
        return;
      }
        ...
        {activeMainTab === "reports" && (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-5">
              ...
              <KpiCard title="Total Expenses" value={kpiData.total_expenses || 0} prefix="Γé╣" decimals={2} loading={loading} icon={<DollarSign className="text-red-500 w-8 h-8" />} />
              ...
              <KpiCard title="Total Salary" value={kpiData.total_salary || 0} prefix="Γé╣" decimals={2} loading={loading} icon={<Briefcase className="text-gray-600 w-8 h-8" />} />
              ...
              <KpiCard title="Service Revenue" value={kpiData.total_service_revenue || 0} prefix="Γé╣" decimals={2} loading={loading} icon={<ConciergeBell className="text-violet-500 w-8 h-8" />} />
              <KpiCard title="Total Purchases" value={kpiData.total_purchases || 0} prefix="Γé╣" decimals={2} loading={loading} icon={<ShoppingBag className="text-amber-500 w-8 h-8" />} />
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-lg font-bold text-green-600">
                            Γé╣<CountUp end={data.assets || 0} duration={1.5} decimals={2} separator="," />
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-600 flex items-center gap-2">
                          ...
                          <span className="text-lg font-bold text-blue-600">
                            Γé╣<CountUp end={data.income || 0} duration={1.5} decimals={2} separator="," />
                          </span>
                        </div>
                          ...
                          <div className="space-y-1 pl-2">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-sm font-semibold text-orange-600">
                                Γé╣<CountUp end={data.regular_expenses || 0} duration={1.5} decimals={2} separator="," />
                              </span>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-xs text-gray-600 flex items-center gap-1">
                              ...
                              <span className="text-sm font-semibold text-red-600">
                                Γé╣<CountUp end={data.inventory_consumption || 0} duration={1.5} decimals={2} separator="," />
                              </span>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-xs text-gray-600 flex items-center gap-1">
                              ...
                              <span className="text-sm font-semibold text-purple-600">
                                Γé╣<CountUp end={data.capital_investment || 0} duration={1.5} decimals={2} separator="," />
                              </span>
                            </div>
                          </div>
                        </div>
                        <div className="mt-3 pt-3 border-t border-gray-200">
                          <div className="flex items-center justify-between">
                            ...
                            <span className={`text-lg font-bold ${(data.income - data.expenses) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                              Γé╣<CountUp end={(data.income || 0) - (data.expenses || 0)} duration={1.5} decimals={2} separator="," />
                            </span>
                          </div>
                        </div>
                      </div>
                  ...
                  >
                    <ResponsiveContainer width="100%" height={300}>
                      <BarChart data={Object.entries(kpiData.department_kpis || {}).map(([dept, data]) => ({
                        ...
                        <YAxis tickFormatter={(value) => `Γé╣${(value / 1000).toFixed(0)}K`} />
                        <Tooltip formatter={(value) => `Γé╣${value.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} />
                      ...
                      <tbody>
                        {filteredLedgers.map((ledger) => (
                          <tr key={ledger.id} className="border-b">
                            ...
                            <td className="px-4 py-2">Γé╣{ledger.opening_balance?.toFixed(2) || "0.00"}</td>
                            <td className="px-4 py-2 font-semibold">
                              <span className={ledger.current_balance >= 0 ? "text-green-600" : "text-red-600"}>
                                Γé╣{Math.abs(ledger.current_balance || 0).toFixed(2)}
              ...
              <div className="bg-white rounded-lg shadow p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    >
                      ≡ƒöº Fix Missing Entries
                    </button>
                    <button
                      onClick={() => {
                        // Export to CSV
                        const csv = [
                          ['Date', 'JE No.', 'Description', 'Ref', 'Debit (Γé╣)', 'Credit (Γé╣)'].join(','),
                          ...journalEntries.flatMap(entry => {
                            ...
                            entry.lines?.forEach(line => {
                              const debit = line.debit_ledger_id ? `Γé╣${line.amount.toFixed(2)}` : '';
                              const credit = line.credit_ledger_id ? `Γé╣${line.amount.toFixed(2)}` : '';
                    ...
                    <table className="min-w-full border-collapse">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="px-4 py-2 text-right">Debit (Γé╣)</th>
                          <th className="px-4 py-2 text-right">Credit (Γé╣)</th>
                        </tr>
                      </thead>
                      <tbody>
                              ...
                              <React.Fragment key={entry.id}>
                                ...
                                >
                                  ...
                                  <td className="px-4 py-2 text-right font-semibold">
                                    {totalDebit > 0 ? `Γé╣${totalDebit.toFixed(2)}` : ''}
                                  </td>
                                  <td className="px-4 py-2 text-right font-semibold">
                                    {totalCredit > 0 ? `Γé╣${totalCredit.toFixed(2)}` : ''}
                                  </td>
                                </tr>
                                {isExpanded && entry.lines && entry.lines.map((line, idx) => (
                                  <tr key={`${entry.id}-line-${idx}`} className="border-b bg-gray-50">
                                    ...
                                    <td className="px-4 py-2 text-right">
                                      {line.debit_ledger_id ? `Γé╣${line.amount.toFixed(2)}` : ''}
                                    </td>
                                    <td className="px-4 py-2 text-right">
                                      {line.credit_ledger_id ? `Γé╣${line.amount.toFixed(2)}` : ''}
                                    </td>
                                  </tr>
                                ))}
                              </React.Fragment>
                  ...
                  <div>
                    <div className="mb-4 flex items-center space-x-4">
                      ...
                      <div className="text-gray-600">
                        Total Debits: Γé╣{trialBalance.total_debits.toFixed(2)} |
                        Total Credits: Γé╣{trialBalance.total_credits.toFixed(2)}
                      </div>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="min-w-full">
                        ...
                        <tbody>
                          {trialBalance.ledgers.map((ledger) => (
                            <tr key={ledger.ledger_id} className="border-b">
                              ...
                              <td className="px-4 py-2 text-right">Γé╣{ledger.debit_total.toFixed(2)}</td>
                              <td className="px-4 py-2 text-right">Γé╣{ledger.credit_total.toFixed(2)}</td>
                              <td className="px-4 py-2 text-right">
                                Γé╣{Math.abs(ledger.balance).toFixed(2)}
                                {ledger.balance < 0 && " (Cr)"}
                              </td>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                        ...
                        <p className="text-2xl font-bold text-green-600">
                          Γé╣{autoReport.summary?.total_revenue?.toFixed(2) || "0.00"}
                        </p>
                      </div>
                      <div className="bg-red-50 rounded-lg p-4 border border-red-200">
                        <h3 className="text-sm font-medium text-red-800 mb-1">Total Expenses</h3>
                        <p className="text-2xl font-bold text-red-600">
                          Γé╣{autoReport.summary?.total_expenses?.toFixed(2) || "0.00"}
                        </p>
                      </div>
                      <div className={`rounded-lg p-4 border ${(autoReport.summary?.net_profit || 0) >= 0
                        ...
                        <p className={`text-2xl font-bold ${(autoReport.summary?.net_profit || 0) >= 0 ? "text-blue-600" : "text-orange-600"
                          ...
                          Γé╣{autoReport.summary?.net_profit?.toFixed(2) || "0.00"}
                        </p>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                          ...
                          <p className="text-lg font-semibold">
                            Γé╣{autoReport.revenue?.checkouts?.grand_total?.toFixed(2) || "0.00"}
                          </p>
                        ...
                        <div>
                          ...
                          <p className="text-lg font-semibold">
                            Γé╣{autoReport.revenue?.checkouts?.room_revenue?.toFixed(2) || "0.00"}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600">Food Revenue</p>
                          <p className="text-lg font-semibold">
                            Γé╣{autoReport.revenue?.checkouts?.food_revenue?.toFixed(2) || "0.00"}
                          </p>
                          <p className="text-xs text-gray-500">
                            + Γé╣{autoReport.revenue?.food_orders?.billed_revenue?.toFixed(2) || "0.00"} (orders)
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-600">Service Revenue</p>
                          <p className="text-lg font-semibold">
                            Γé╣{autoReport.revenue?.checkouts?.service_revenue?.toFixed(2) || "0.00"}
                          </p>
                          <p className="text-xs text-gray-500">
                            + Γé╣{autoReport.revenue?.services?.billed_revenue?.toFixed(2) || "0.00"} (services)
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          ...
                          <p className="text-lg font-semibold">
                            Γé╣{autoReport.expenses?.operating_expenses?.total_amount?.toFixed(2) || "0.00"}
                          </p>
                          <p className="text-xs text-gray-500">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-semibold">
                            Γé╣{autoReport.expenses?.inventory_purchases?.total_amount?.toFixed(2) || "0.00"}
                          </p>
                          <p className="text-xs text-gray-500">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-semibold">
                            Γé╣{autoReport.expenses?.inventory_consumption?.total_cogs?.toFixed(2) || "0.00"}
                          </p>
                          <p className="text-xs text-gray-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="bg-white border rounded-lg p-4">
                        ...
                        <ul className="text-sm space-y-1">
                          <li>Tax Collected: Γé╣{autoReport.revenue?.checkouts?.tax_collected?.toFixed(2) || "0.00"}</li>
                          <li>Discount Given: Γé╣{autoReport.revenue?.checkouts?.discount_given?.toFixed(2) || "0.00"}</li>
                          <li>Consumables: Γé╣{autoReport.revenue?.checkouts?.consumables_charges?.toFixed(2) || "0.00"}</li>
                          <li>Asset Damage: Γé╣{autoReport.revenue?.checkouts?.asset_damage_charges?.toFixed(2) || "0.00"}</li>
                          <li>Tips/Gratuity: Γé╣{autoReport.revenue?.checkouts?.tips_gratuity?.toFixed(2) || "0.00"}</li>
                        </ul>
                      </div>
                      <div className="bg-white border rounded-lg p-4">
                        ...
                        <ul className="text-sm space-y-1">
                          ...
                          <li>Billed Revenue: Γé╣{autoReport.revenue?.food_orders?.billed_revenue?.toFixed(2) || "0.00"}</li>
                          <li>Unbilled Revenue: Γé╣{autoReport.revenue?.food_orders?.unbilled_revenue?.toFixed(2) || "0.00"}</li>
                        </ul>
                      </div>
                    </div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                            {Object.entries(autoReport.expenses.operating_expenses.by_category).map(([category, data]) => (
                              <div key={category} className="text-sm">
                                ...
                                <p className="text-gray-600">Γé╣{data.amount?.toFixed(2) || "0.00"}</p>
                    ...
                    <div className="bg-indigo-50 rounded-lg p-4 border border-indigo-200">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <p className="text-xl font-bold text-red-600">Γé╣{comprehensiveReport.summary?.total_salary_cost?.toFixed(2) || "0.00"}</p>
                        </div>
                      </div>
                    </div>
                          ...
                          <tbody>
                            {comprehensiveReport.data?.checkouts?.slice(0, 50).map((c) => (
                              <tr key={c.id} className="border-b">
                                ...
                                <td className="p-2 text-right">Γé╣{c.grand_total?.toFixed(2)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                          ...
                          <tbody>
                            {comprehensiveReport.data?.bookings?.slice(0, 50).map((b) => (
                              <tr key={b.id} className="border-b">
                                ...
                                <td className="p-2 text-right">Γé╣{b.total_amount?.toFixed(2)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                          ...
                          <tbody>
                            {comprehensiveReport.data?.food_orders?.slice(0, 50).map((fo) => (
                              <tr key={fo.id} className="border-b">
                                ...
                                <td className="p-2 text-right">Γé╣{fo.amount?.toFixed(2)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                          ...
                          <tbody>
                            {comprehensiveReport.data?.expenses?.slice(0, 50).map((e) => (
                              <tr key={e.id} className="border-b">
                                ...
                                <td className="p-2 text-right">Γé╣{e.amount?.toFixed(2)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                          ...
                          <tbody>
                            {comprehensiveReport.data?.purchases?.slice(0, 50).map((p) => (
                              <tr key={p.id} className="border-b">
                                ...
                                <td className="p-2 text-right">Γé╣{p.total_amount?.toFixed(2)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                          ...
                          <tbody>
                            {comprehensiveReport.data?.inventory_transactions?.slice(0, 50).map((t) => (
                              <tr key={t.id} className="border-b">
                                ...
                                <td className="p-2 text-right">Γé╣{t.total_amount?.toFixed(2)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                          ...
                          <tbody>
                            {comprehensiveReport.data?.employees?.slice(0, 50).map((e) => (
                              <tr key={e.id} className="border-b">
                                ...
                                <td className="p-2 text-right font-semibold">Γé╣{e.salary?.toFixed(2)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                          <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                            ...
                            <p className="text-2xl font-bold text-green-600">
                              Γé╣{((gstReportData.total_output_tax?.total ?? gstReportData.total_output_tax) || 0).toFixed(2)}
                            </p>
                          </div>
                          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                            <h3 className="text-sm font-medium text-blue-800 mb-1">Input Tax Credit</h3>
                            <p className="text-2xl font-bold text-blue-600">
                              Γé╣{((gstReportData.total_input_tax_credit?.total ?? gstReportData.input_tax_credit ?? gstReportData.total_input_tax_credit) || 0).toFixed(2)}
                            </p>
                          </div>
                          <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
                            <h3 className="text-sm font-medium text-orange-800 mb-1">RCM Liability</h3>
                            <p className="text-2xl font-bold text-orange-600">
                              Γé╣{((gstReportData.total_rcm_liability ?? gstReportData.rcm_liability) || 0).toFixed(2)}
                            </p>
                          </div>
                          <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                            <h3 className="text-sm font-medium text-purple-800 mb-1">Net Payable</h3>
                            <p className="text-2xl font-bold text-purple-600">
                              Γé╣{((gstReportData.net_gst_payable ?? gstReportData.net_cash_ledger_payable) || 0).toFixed(2)}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        <div className="mb-4 p-4 bg-gray-50 rounded grid grid-cols-2 md:grid-cols-4 gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-lg font-bold">Γé╣{gstReportData.total_taxable_value?.toFixed(2) || "0.00"}</p>
                          </div>
                            ...
                            <tbody>
                              {gstReportData.data?.map((item, idx) => (
                                <tr key={idx} className="hover:bg-gray-50">
                                  ...
                                  <td className="p-2 border text-right">Γé╣{item.invoice_value?.toFixed(2)}</td>
                                  ...
                                  <td className="p-2 border text-right">Γé╣{item.taxable_value?.toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{item.igst?.toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{item.cgst?.toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{item.sgst?.toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{item.cess?.toFixed(2)}</td>
                            ...
                            {gstReportData.data && gstReportData.data.length > 0 && (
                              <tfoot className="bg-gray-100 font-bold">
                                <tr>
                                  ...
                                  <td className="p-2 border text-right">Γé╣{gstReportData.total_taxable_value?.toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{gstReportData.total_igst?.toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{gstReportData.total_cgst?.toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{gstReportData.total_sgst?.toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{gstReportData.total_cess?.toFixed(2)}</td>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded">
                          ...
                          <div>
                            ...
                            <p className="text-lg font-bold">Γé╣{gstReportData.summary?.total_b2c_large_taxable?.toFixed(2) || "0.00"}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-600">B2C Small Taxable</p>
                            <p className="text-lg font-bold">Γé╣{gstReportData.summary?.total_b2c_small_taxable?.toFixed(2) || "0.00"}</p>
                          </div>
                        </div>
                        ...
                        <div>
                          <div className="mb-4 p-4 bg-blue-50 rounded border border-blue-200">
                            <h3 className="text-lg font-bold mb-2">{gstReportData.b2c_large?.description || "B2C Large (Inter-State Invoices > Γé╣2.5L)"}</h3>
                            <p className="text-sm text-blue-800">
                              These are inter-state invoices with value exceeding Γé╣2.5 Lakhs. Must be reported invoice-by-invoice.
                            </p>
                          </div>
                          {gstReportData.b2c_large?.data && gstReportData.b2c_large.data.length > 0 ? (
                            ...
                            <div className="text-center py-8 text-gray-500 bg-gray-50 rounded">
                              No B2C Large invoices found (Inter-state invoices &gt; Γé╣2.5L)
                            </div>
                          )}
                        </div>
                        ...
                        <div>
                          <div className="mb-4 p-4 bg-green-50 rounded border border-green-200">
                            ...
                            <p className="text-sm text-green-800">
                              All intra-state sales and inter-state sales below Γé╣2.5L are grouped by Place of Supply and Tax Rate.
                                ...
                                <tbody>
                                  {gstReportData.b2c_small.data.map((item, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50">
                                      <td className="p-2 border">{item.place_of_supply}</td>
                                      <td className="p-2 border text-right">{item.rate}%</td>
                                      <td className="p-2 border text-right font-semibold">Γé╣{item.taxable_value?.toFixed(2)}</td>
                                      <td className="p-2 border text-right">Γé╣{item.igst?.toFixed(2)}</td>
                                      <td className="p-2 border text-right">Γé╣{item.cgst?.toFixed(2)}</td>
                                      <td className="p-2 border text-right">Γé╣{item.sgst?.toFixed(2)}</td>
                                      <td className="p-2 border text-right">Γé╣{item.cess?.toFixed(2)}</td>
                                ...
                                {gstReportData.b2c_small.data.length > 0 && (
                                  <tfoot className="bg-gray-100 font-bold">
                                    <tr>
                                      ...
                                      <td className="p-2 border text-right">Γé╣{gstReportData.summary?.total_b2c_small_taxable?.toFixed(2)}</td>
                                      <td className="p-2 border text-right">Γé╣{gstReportData.summary?.total_b2c_small_igst?.toFixed(2)}</td>
                                      <td className="p-2 border text-right">Γé╣{gstReportData.summary?.total_b2c_small_cgst?.toFixed(2)}</td>
                                      <td className="p-2 border text-right">Γé╣{gstReportData.summary?.total_b2c_small_sgst?.toFixed(2)}</td>
                                      <td className="p-2 border text-right">Γé╣{gstReportData.b2c_small.data.reduce((sum, item) => sum + (item.cess || 0), 0).toFixed(2)}</td>
                        ...
                        {gstReportData.summary && (
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                              ...
                              <p className="text-2xl font-bold text-green-600">Γé╣{((gstReportData.summary?.total_value || 0)).toFixed(2)}</p>
                            </div>
                            <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                              <h3 className="text-sm font-medium text-purple-800 mb-1">Total Taxable</h3>
                              <p className="text-2xl font-bold text-purple-600">Γé╣{((gstReportData.summary?.total_taxable_value || 0)).toFixed(2)}</p>
                            </div>
                            <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
                              <h3 className="text-sm font-medium text-orange-800 mb-1">Total Tax</h3>
                              <p className="text-2xl font-bold text-orange-600">Γé╣{((gstReportData.summary?.total_integrated_tax || 0) + (gstReportData.summary?.total_central_tax || 0) + (gstReportData.summary?.total_state_ut_tax || 0)).toFixed(2)}</p>
                            </div>
                          </div>
                        )}
                            ...
                            <tbody>
                              {gstReportData.data?.map((item, idx) => (
                                <tr key={idx} className="hover:bg-gray-50">
                                  ...
                                  <td className="p-2 border text-right">Γé╣{((item.total_value || 0)).toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{((item.taxable_value || 0)).toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{((item.integrated_tax || item.igst || 0)).toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{((item.central_tax || item.cgst || 0)).toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{((item.state_ut_tax || item.sgst || 0)).toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{((item.cess_amount || item.cess || 0)).toFixed(2)}</td>
                                </tr>
                              ))}
                            </tbody>
                            {gstReportData.summary && (
                              <tfoot>
                                <tr className="bg-gray-200 font-bold">
                                  ...
                                  <td className="p-2 border text-right">Γé╣{((gstReportData.summary.total_value || 0)).toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{((gstReportData.summary.total_taxable_value || 0)).toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{((gstReportData.summary.total_integrated_tax || 0)).toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{((gstReportData.summary.total_central_tax || 0)).toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{((gstReportData.summary.total_state_ut_tax || 0)).toFixed(2)}</td>
                                  <td className="p-2 border text-right">Γé╣{((gstReportData.summary.total_cess || 0)).toFixed(2)}</td>
                                </tr>
                              </tfoot>
                            )}
                          </table>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                            ...
                            <p className="text-2xl font-bold text-green-600">Γé╣{gstReportData.summary?.total_eligible_itc?.toFixed(2) || "0.00"}</p>
                          ...
                          <div className="bg-red-50 rounded-lg p-4 border border-red-200">
                            ...
                            <p className="text-2xl font-bold text-red-600">Γé╣{gstReportData.summary?.total_ineligible_itc?.toFixed(2) || "0.00"}</p>
                          ...
                          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                            ...
                            <p className="text-2xl font-bold text-blue-600">Γé╣{gstReportData.summary?.net_claimable_itc?.toFixed(2) || "0.00"}</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                          <div className="bg-white border rounded p-3">
                            ...
                            <p className="text-lg font-bold">Γé╣{gstReportData.input_goods?.total_tax?.toFixed(2) || "0.00"}</p>
                          ...
                          <div className="bg-white border rounded p-3">
                            ...
                            <p className="text-lg font-bold">Γé╣{gstReportData.capital_goods?.total_tax?.toFixed(2) || "0.00"}</p>
                          ...
                          <div className="bg-white border rounded p-3">
                            ...
                            <p className="text-lg font-bold">Γé╣{gstReportData.input_services?.total_tax?.toFixed(2) || "0.00"}</p>
                          ...
                          <div className="bg-white border rounded p-3 border-red-300">
                            ...
                            <p className="text-lg font-bold text-red-600">Γé╣{gstReportData.ineligible?.total_tax?.toFixed(2) || "0.00"}</p>
                              ...
                              <tbody>
                                {gstReportData.all_eligible?.data?.map((item, idx) => (
                                  <tr key={`itc-${item.invoice_number || idx}-${item.vendor_gstin || idx}-${idx}`} className="hover:bg-gray-50">
                                    ...
                                    <td className="p-2 border text-right">Γé╣{item.cgst?.toFixed(2)}</td>
                                    <td className="p-2 border text-right">Γé╣{item.sgst?.toFixed(2)}</td>
                                    <td className="p-2 border text-right font-semibold">Γé╣{item.total_tax?.toFixed(2)}</td>
                                  </tr>
                                ))}
                              </tbody>
                              {gstReportData.all_eligible?.data && gstReportData.all_eligible.data.length > 0 && (
                                <tfoot className="bg-gray-100 font-bold">
                                  <tr>
                                    ...
                                    <td className="p-2 border text-right">Γé╣{gstReportData.all_eligible?.total_taxable_value?.toFixed(2)}</td>
                                    <td className="p-2 border text-right">Γé╣{gstReportData.all_eligible?.total_igst?.toFixed(2)}</td>
                                    <td className="p-2 border text-right">Γé╣{gstReportData.all_eligible?.total_cgst?.toFixed(2)}</td>
                                    <td className="p-2 border text-right">Γé╣{gstReportData.all_eligible?.total_sgst?.toFixed(2)}</td>
                                    <td className="p-2 border text-right">Γé╣{gstReportData.all_eligible?.total_tax?.toFixed(2)}</td>
                                  </tr>
                                </tfoot>
                              )}
                            </table>
                              ...
                              <table className="min-w-full text-xs border">
                                ...
                                <tbody>
                                  {gstReportData.ineligible.data.map((item, idx) => (
                                    <tr key={idx} className="hover:bg-red-50">
                                      ...
                                      <td className="p-2 border text-xs">{item.category_name}</td>
                                      <td className="p-2 border text-right">Γé╣{item.taxable_value?.toFixed(2)}</td>
                                      <td className="p-2 border text-right font-semibold text-red-600">Γé╣{item.total_tax?.toFixed(2)}</td>
                                ...
                                <tfoot className="bg-red-100 font-bold">
                                  <tr>
                                    ...
                                    <td className="p-2 border text-right">Γé╣{gstReportData.ineligible?.total_taxable_value?.toFixed(2)}</td>
                                    <td className="p-2 border text-right text-red-600">Γé╣{gstReportData.ineligible?.total_tax?.toFixed(2)}</td>
                                    <td className="p-2 border"></td>
                                  </tr>
                                </tfoot>
                              </table>
                              ...
                              {gstr2bReconcileData.unmatched_in_gstr2b && gstr2bReconcileData.unmatched_in_gstr2b.length > 0 && (
                                <div>
                                  <h4 className="font-bold text-red-700 mb-2">
                                    ΓÜá∩╕Å Invoices in GSTR-2B but NOT in Your System ({gstr2bReconcileData.unmatched_in_gstr2b.length})
                                  </h4>
                              ...
                              {gstr2bReconcileData.unmatched_in_system && gstr2bReconcileData.unmatched_in_system.length > 0 && (
                                <div>
                                  <h4 className="font-bold text-orange-700 mb-2">
                                    ΓÜá∩╕Å Invoices in Your System but NOT in GSTR-2B ({gstr2bReconcileData.unmatched_in_system.length})
                                  </h4>
                                      ...
                                      <tbody>
                                        {gstr2bReconcileData.unmatched_in_system.map((item, idx) => (
                                          <tr key={idx} className="bg-white">
                                            ...
                                            <td className="p-2 border text-right">Γé╣{item.total_amount?.toFixed(2)}</td>
                              ...
                              {gstr2bReconcileData.matched && gstr2bReconcileData.matched.length > 0 && (
                                <div>
                                  <h4 className="font-bold text-green-700 mb-2">
                                    Γ£à Matched Invoices ({gstr2bReconcileData.matched.length})
                                  </h4>
                                      ...
                                      <tbody>
                                        {gstr2bReconcileData.matched.map((item, idx) => (
                                          <tr key={idx} className="bg-white">
                                            ...
                                            <td className="p-2 border">{item.vendor_name || "-"}</td>
                                            <td className="p-2 border text-right">Γé╣{item.total_amount?.toFixed(2)}</td>
                                            <td className="p-2 border text-green-600">Γ£ô Matched</td>
                                          </tr>
                                        ))}
                                      </tbody>
                                    </table>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
                            ...
                            <p className="text-2xl font-bold text-orange-600">Γé╣{gstReportData.summary?.total_tax_liability?.toFixed(2) || "0.00"}</p>
                          ...
                          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                            ...
                            <p className="text-2xl font-bold text-blue-600">Γé╣{gstReportData.summary?.total_taxable_value?.toFixed(2) || "0.00"}</p>
                          </div>
                          <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                          ...
                          <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                            ...
                            <p className="text-2xl font-bold text-purple-600">Γé╣{gstReportData.summary?.total_igst?.toFixed(2) || "0.00"}</p>
                          </div>
                        </div>
                            ...
                            <tbody>
                              {gstReportData.data?.map((item, idx) => (
                                <tr key={idx} className="hover:bg-gray-50">
                                  ...
                                  <td className="p-2 border text-right">Γé╣{item.taxable_value?.toFixed(2)}</td>
                                  <td className="p-2 border text-right">{item.tax_rate}%</td>
                                  <td className="p-2 border text-right font-bold text-orange-600">Γé╣{item.tax_liability?.toFixed(2)}</td>
                                  <td className="p-2 border">
                            ...
                            {gstReportData.data && gstReportData.data.length > 0 && (
                              <tfoot className="bg-gray-100 font-bold">
                                <tr>
                                  ...
                                  <td className="p-2 border text-right">Γé╣{gstReportData.summary?.total_taxable_value?.toFixed(2)}</td>
                                  <td className="p-2 border"></td>
                                  <td className="p-2 border text-right text-orange-600">Γé╣{gstReportData.summary?.total_tax_liability?.toFixed(2)}</td>
                                  <td colSpan="2" className="p-2 border"></td>
                                </tr>
                              </tfoot>
                            )}
                      ...
                      <div>
                        <div className="mb-4 grid grid-cols-3 gap-4">
                          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                            ...
                            <p className="text-xl font-bold">Γé╣{gstReportData.total_advance_received?.toFixed(2) || "0.00"}</p>
                          </div>
                          <div className="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
                            <p className="text-sm font-medium">Unadjusted</p>
                            <p className="text-xl font-bold">Γé╣{gstReportData.unadjusted_advance?.toFixed(2) || "0.00"}</p>
                          </div>
                          <div className="bg-red-50 rounded-lg p-4 border border-red-200">
                            <p className="text-sm font-medium">Tax Liability</p>
                            <p className="text-xl font-bold">Γé╣{gstReportData.tax_liability_on_advance?.toFixed(2) || "0.00"}</p>
                          </div>
                        </div>
                        <div className="overflow-x-auto">
                          <table className="min-w-full text-sm border">
                            ...
                            <tbody>
                              {gstReportData.data?.map((item, idx) => (
                                <tr key={idx}>
                                  ...
                                  <td className="p-2 border text-right">Γé╣{item.advance_amount?.toFixed(2)}</td>
                                  <td className="p-2 border">{item.invoice_raised ? "Yes" : "No"}</td>
                                  <td className="p-2 border text-right">Γé╣{item.unadjusted_advance?.toFixed(2)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                      ...
                      <div className="space-y-6">
                        ...
                        {gstReportData.slab_5_percent && (
                          <div>
                            ...
                            <div className="mb-4 p-4 bg-gray-50 rounded">
                              ...
                              <p className="text-sm"><strong>Total Revenue:</strong> Γé╣{gstReportData.slab_5_percent?.total_revenue?.toFixed(2) || "0.00"}</p>
                            </div>
                            <div className="overflow-x-auto">
                              <table className="min-w-full text-sm border">
                                <thead>
                                ...
                                <tbody>
                                  {gstReportData.slab_5_percent?.data?.slice(0, 50).map((item, idx) => (
                                    <tr key={`slab5-${item.invoice_number || idx}-${item.invoice_date || idx}`}>
                                      ...
                                      <td className="p-2 border">{item.guest_name}</td>
                                      <td className="p-2 border text-right">Γé╣{item.room_total?.toFixed(2)}</td>
                                      <td className="p-2 border text-right">{item.expected_tax_rate}%</td>
                                      <td className="p-2 border text-right">Γé╣{item.tax_applied?.toFixed(2)}</td>
                        ...
                        <div>
                          ...
                          <div className="mb-4 p-4 bg-gray-50 rounded">
                            ...
                            <p className="text-sm"><strong>Total Revenue:</strong> Γé╣{gstReportData.slab_12_percent?.total_revenue?.toFixed(2) || "0.00"}</p>
                          </div>
                          <div className="overflow-x-auto">
                            <table className="min-w-full text-sm border">
                              <thead>
                        ...
                        <div>
                          ...
                          <div className="mb-4 p-4 bg-gray-50 rounded">
                            ...
                            <p className="text-sm"><strong>Total Revenue:</strong> Γé╣{gstReportData.slab_18_percent?.total_revenue?.toFixed(2) || "0.00"}</p>
                          </div>
                          <div className="overflow-x-auto">
                            <table className="min-w-full text-sm border">
                              <thead>
                    ...
                    <div className="mt-4 p-3 bg-gray-100 rounded border">
                      {(() => {
                        ...
                        return (
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-4">
                              <span className="text-sm font-medium">Total Debits: Γé╣{totalDebits.toFixed(2)}</span>
                              <span className="text-sm font-medium">Total Credits: Γé╣{totalCredits.toFixed(2)}</span>
                            </div>
                            {isBalanced ? (
                              ...
                              <span className="text-red-600 font-semibold flex items-center">
                                ...
                                Not Balanced (Difference: Γé╣{Math.abs(totalDebits - totalCredits).toFixed(2)})
                              </span>
                            )}
                          </div>
                        );
